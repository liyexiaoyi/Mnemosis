"""Final matrix v6 chart: 23 dimensions x 4 projects x 3 models."""
...
def chart() -> str:
    matrix = json.load(
        open(os.path.join(_BENCH, "results", "final_matrix_v6.json"),
             encoding="utf-8")
    )
    ...
    dim_labels = {
        ...
        "self_reference": "自我参照(新)",
        "source_monitoring": "来源监控(新)",
        "mood_congruent": "情绪一致(新)",
        "confidence_weighting": "置信度加权(新)",
    }
    retr_keys = {
        "en12": "en12",
        "zh16": "zh16_premises",
    ...
    f_txt = _font(15)
    y = 28
    draw.text((42, y), "第 6 期全方位测评：Mnemosis vs GitHub 同类项目（23 维度）",
              fill="#111", font=f_title)
    y += 48
    draw.text((42, y),
              "覆盖第 64-67 轮新增能力（自我参照/来源监控/情绪一致/置信度加权）；"
    ...
    draw.text((42, y + 8),
              "读法：A 检索矩阵含 17 个新能力维度（Mnemosis 独有，第三方项目不支持）；"
              "B/C 显示三模型作答——换强模型能缩小差距，但检索短板的项目上限仍低。",
              fill="#555", font=f_sub)
    draw.text((42, y + 42),
              "说明：基础 6 维沿用第 4/5 期真实安装测评；59-67 轮改动已跑统一回归"
              "（en88/zh200/zh10k 及 10k 系列）零差异，故基础行不变。",
              fill="#555", font=f_sub)

    path = os.path.join(_OUT, "final_full_matrix_v6.png")
    img.save(path)
    return path
