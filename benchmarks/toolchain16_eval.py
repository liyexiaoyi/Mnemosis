"""Toolchain panorama 16 (round 182).

Extends the 55-step run with dependency_map, project_risk and
plan_tracker (58 steps).
"""
...
def _run() -> dict:
    ...
    batch = fresh_server._call_tool(
        "search_batch",
        {"queries": ["t16-0", "t16-1", "t16-2"], "top_k": 3},
    )
    batch_ok = sum(
        1
    ...
    pack = fresh_server._call_tool(
        "context_pack",
        {"queries": ["t16-0", "t16-1", "t16-2"], "max_chars": 400},
    )
    pack_ok = int(
...
def main() -> int:
    ...
    parser.add_argument(
        "--out",
        default=os.path.join(_BENCH, "results", "toolchain16_eval.json"),
    )
