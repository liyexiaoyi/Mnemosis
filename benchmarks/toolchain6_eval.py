"""Toolchain panorama 6 (round 132).

Extends the 18-step run with prospective-memory intents (remember/due/
complete/report), retrieval_assist and schema_report: export -> import ->
status -> audit -> review_load -> dedupe -> resolve_conflicts -> tag ->
cleanup_preview -> recall_log -> similarity_report -> association_report ->
search_batch -> intent_remember -> intent_due -> intent_complete ->
intent_report -> retrieval_assist -> schema_report -> review_batch ->
practice_session -> sleep_and_plan -> search -> list_conflicts ->
practice_forecast (25 steps).
"""
...
def _run() -> dict:
    ...
    batch = fresh_server._call_tool(
        "search_batch",
        {"queries": ["t6-0", "t6-1", "t6-2"], "top_k": 3},
    )
    batch_ok = sum(
        1
...
def main() -> int:
    ...
    parser.add_argument(
        "--out",
        default=os.path.join(_BENCH, "results", "toolchain6_eval.json"),
    )
