"""Final matrix v35 chart: 139 dimensions x 4 projects x 3 models."""
...
def chart() -> str:
    matrix = json.load(
        open(os.path.join(_BENCH, "results", "final_matrix_v35.json"),
             encoding="utf-8")
    )
    ...
    f_txt = _font(16)
    y = 28
    draw.text((42, y), "第 35 期全方位测评：Mnemosis vs GitHub 同类项目（139 维度）",
              fill="#111", font=f_title)
    y += 48
    draw.text((42, y),
              "覆盖第 208-211 轮新增能力（睡眠推断/图式拟合/工作预算/工具链22）；"
    ...
    draw.text((42, y + 8),
              ...
              fill="#555", font=f_sub)
    draw.text((42, y + 42),
              "说明：基础维度沿用第 4-34 期真实安装测评；59-211 轮改动已跑统一回归"
              "（en88/zh200/zh10k 及 10k 系列）与一键全测评（125/125）零差异。",
              fill="#555", font=f_sub)

    path = os.path.join(_OUT, "final_full_matrix_v35.png")
    img.save(path)
    return path
