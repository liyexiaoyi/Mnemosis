"""Toolchain panorama 12 (round 162).

Extends the 42-step run with multi_hop_report, cramming_plan and
session_summary (45 steps).
"""
...
def _run() -> dict:
    ...
    batch = fresh_server._call_tool(
        "search_batch",
        {"queries": ["t12-0", "t12-1", "t12-2"], "top_k": 3},
    )
    batch_ok = sum(
        1
    ...
    pack = fresh_server._call_tool(
        "context_pack",
        {"queries": ["t12-0", "t12-1", "t12-2"], "max_chars": 400},
    )
    pack_ok = int(
...
def main() -> int:
    ...
    parser.add_argument(
        "--out",
        default=os.path.join(_BENCH, "results", "toolchain12_eval.json"),
    )
