"""Toolchain panorama 18 (round 192).

Extends the 61-step run with decision_review, transfer_report and
retrieval_quality (64 steps).
"""
...
def _run() -> dict:
    ...
    batch = fresh_server._call_tool(
        "search_batch",
        {"queries": ["t18-0", "t18-1", "t18-2"], "top_k": 3},
    )
    batch_ok = sum(
        1
    ...
    pack = fresh_server._call_tool(
        "context_pack",
        {"queries": ["t18-0", "t18-1", "t18-2"], "max_chars": 400},
    )
    pack_ok = int(
    ...
    rq = fresh_server._call_tool(
        "retrieval_quality",
        {"queries": ["t18-0", "t18-1", "zzz miss"], "top_k": 3},
    )
    rq_ok = int(
        isinstance(rq, dict)
...
def main() -> int:
    ...
    parser.add_argument(
        "--out",
        default=os.path.join(_BENCH, "results", "toolchain18_eval.json"),
    )
