"""Toolchain panorama 10 (round 152).

Extends the 36-step run with context_pack, encoding_quality and
explain_memory (39 steps).
"""
...
def _run() -> dict:
    ...
    batch = fresh_server._call_tool(
        "search_batch",
        {"queries": ["t10-0", "t10-1", "t10-2"], "top_k": 3},
    )
    batch_ok = sum(
        1
    ...
    pack = fresh_server._call_tool(
        "context_pack",
        {"queries": ["t10-0", "t10-1", "t10-2"], "max_chars": 400},
    )
    pack_ok = int(
...
def main() -> int:
    ...
    parser.add_argument(
        "--out",
        default=os.path.join(_BENCH, "results", "toolchain10_eval.json"),
    )
