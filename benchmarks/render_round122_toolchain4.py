"""Round-122 chart: toolchain panorama 4."""
...
def chart() -> str:
    ...
    draw.text((42, 26), "第 122 轮：工具链全景回归 4（15 步全过）",
              fill="#111", font=f_title)
    draw.text((42, 74),
              "依据：索引维护（标签）、审计（检索日志）、清理预览也必须与"
              "既有工具在同一份数据上连续工作。",
              fill="#555", font=f_sub)
    ...
    base_y = 390
    chart_h = 230
    draw.text((x0, 120), "① 工具链 15 步结果",
              fill="#111", font=f_panel)
    draw.line([(x0, base_y), (x0 + 1500, base_y)], fill="#999", width=2)
    for frac, label in ((0.0, "0"), (3.0, "3"), (6.0, "6")):
        y = base_y - frac / 6.0 * chart_h
        draw.line([(x0, y), (x0 + 1500, y)], fill="#e5e5e5", width=1)
    ...
    for i, (name, val, total, color) in enumerate(steps):
        ...
        draw.text((bx + 14, base_y - bh + 6), f"{val}/{total}",
                  fill="white", font=f_val)
        draw.text((bx - 8, base_y + 10), name, fill="#111", font=f_label)

    draw.text((42, 600),
              "怎么看：同一份记忆走完 15 步（标签 6 条、清理预览、检索日志等）"
              "全部正确，检索 6/6、冲突清零。",
              fill="#555", font=f_note)
    draw.text((42, 640),
              "一键全测评扩到 57/57——索引与审计工具没有破坏任何既有机制。",
              fill="#555", font=f_note)
    draw.text((42, 710),
              "回归：226 单元测试全过 + 一键全测评 57/57。",
              fill="#555", font=f_note)

    path = os.path.join(_OUT, "round122_toolchain4.png")
    img.save(path)
    return path
