"""Round-262 chart: pet & family-life spot-check, Mnemosis vs mem0 only."""
...
def chart() -> str:
    data = json.load(
        open(os.path.join(_WORK, "pet_spot.json"), encoding="utf-8")
    )
    ...
    draw.text(
        (42, 26),
        "第 262 轮抽查：宠物家庭记忆对比（只对比 mem0 官方）",
        fill="#111",
        font=f_title,
    )
    draw.text(
        (42, 72),
        "内容：两只猫 + 家庭生活的 24 条记忆（宠物档案/喂食/疫苗/用药/"
        "医院/家庭规则/购物/学校/维修/节日），含“改剂量”“还没打”等易混题。",
        fill="#555",
        font=f_sub,
    )
    ...
    draw.text(
        (42, y + 40),
        "过程说明：本轮一次通过；否定题（狂犬还没打）和剂量变更题"
        "（5mg→10mg）两家都答对，没有发现弱项。",
        fill="#555",
        font=f_sub,
    )
    draw.text(
        (42, y + 80),
        "结论：第 262 轮 Mnemosis 10/10 vs mem0 10/10，无弱项；"
        "继续换领域找差距。",
        fill="#555",
        font=f_sub,
    )

    path = os.path.join(_OUT, "round262_pet.png")
    img.save(path)
    return path
