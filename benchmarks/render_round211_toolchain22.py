"""Round-211 chart: toolchain panorama 22 (horizontal, readable)."""
...
def chart() -> str:
    ...
    draw.text((42, 26), "第 211 轮：工具链全景回归 22（75 步全过）",
              fill="#111", font=f_title)
    draw.text((42, 74),
              "同一份记忆连续走完 75 步（含睡眠推断/图式拟合/工作预算），全部正确；"
              "搜索 6/6、批量检索 3/3。",
              fill="#555", font=f_sub)
    max_total = 6
    bar_x0 = 330
    ...
    for i, (name, val, total) in enumerate(steps):
        ...
        y += row_h
    draw.text((42, y + 14),
              "怎么看：75 行全绿——旧功能没坏，第 208-210 轮的睡眠推断/图式拟合/"
              "工作预算也在同一套数据上正常。",
              fill="#555", font=f_note)
    draw.text((42, y + 48),
              "回归：280 单元测试全过 + 一键全测评全绿。",
              fill="#555", font=f_note)

    path = os.path.join(_OUT, "round211_toolchain22.png")
    img.save(path)
    return path
