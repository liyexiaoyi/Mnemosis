"""Round-241 chart: toolchain panorama 28 (horizontal, readable)."""
...
def chart() -> str:
    toolchain = json.load(
        open(
            os.path.join(_BENCH, "results", "toolchain28_eval.json"),
            encoding="utf-8",
        )
    )
    steps = [
    ...
    draw.text((42, 26), "第 241 轮：工具链全景回归 28（93 步全过）",
              fill="#111", font=f_title)
    draw.text((42, 74),
              "同一份记忆连续走完 93 步（含遗忘曲线/情绪衰减/目标进度），全部正确；"
              "搜索 6/6、批量检索 3/3。",
              fill="#555", font=f_sub)
    max_total = 6
    bar_x0 = 330
    ...
    for i, (name, val, total) in enumerate(steps):
        ...
        y += row_h
    draw.text((42, y + 14),
              "怎么看：93 行全绿——旧功能没坏，第 238-240 轮的遗忘曲线/情绪衰减/"
              "目标进度也在同一套数据上正常。",
              fill="#555", font=f_note)
    draw.text((42, y + 48),
              "回归：298 单元测试全过 + 一键全测评全绿。",
              fill="#555", font=f_note)

    path = os.path.join(_OUT, "round241_toolchain28.png")
    img.save(path)
    return path
