"""Toolchain panorama 17 (round 187).

Extends the 58-step run with plan_rewrite, lesson_learned and
effort_estimate (61 steps).
"""
...
def _run() -> dict:
    ...
    batch = fresh_server._call_tool(
        "search_batch",
        {"queries": ["t17-0", "t17-1", "t17-2"], "top_k": 3},
    )
    batch_ok = sum(
        1
    ...
    pack = fresh_server._call_tool(
        "context_pack",
        {"queries": ["t17-0", "t17-1", "t17-2"], "max_chars": 400},
    )
    pack_ok = int(
...
def main() -> int:
    ...
    parser.add_argument(
        "--out",
        default=os.path.join(_BENCH, "results", "toolchain17_eval.json"),
    )
