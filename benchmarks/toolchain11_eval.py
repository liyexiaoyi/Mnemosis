"""Toolchain panorama 11 (round 157).

Extends the 39-step run with compare_memories, action_queue and
summarize_cluster (42 steps).
"""
...
def _run() -> dict:
    ...
    batch = fresh_server._call_tool(
        "search_batch",
        {"queries": ["t11-0", "t11-1", "t11-2"], "top_k": 3},
    )
    batch_ok = sum(
        1
    ...
    pack = fresh_server._call_tool(
        "context_pack",
        {"queries": ["t11-0", "t11-1", "t11-2"], "max_chars": 400},
    )
    pack_ok = int(
...
def main() -> int:
    ...
    parser.add_argument(
        "--out",
        default=os.path.join(_BENCH, "results", "toolchain11_eval.json"),
    )
