"""Round-265 local-model re-test.

Same job-hunting dataset, same retrieval contexts already saved in
work/job_spot.json (Mnemosis vs mem0 top-4), but the answer model is a
local small model (qwen2.5:3b / gemma3:12b via Ollama) instead of the
cloud model. Scoring uses the same hit rules.
"""
    ...
    for model in MODELS:
        ...
        for project in ("mnemosis", "mem0"):
            ...
            for question in QUESTIONS:
                ...
                print(
                    f"  [{model}][{project}] {question['q'][:20]} done",
                    flush=True,
                )
        accuracy = {}
        ...
        key = "answers_local_" + model.replace(":", "_")
        acc_key = "accuracy_local_" + model.replace(":", "_")
        data[key] = answers
        data[acc_key] = accuracy
        with open(
            os.path.join(_WORK, f"job_local_{model.replace(':', '_')}.json"),
            "w",
            encoding="utf-8",
        ) as handle:
