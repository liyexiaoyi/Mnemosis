"""Toolchain panorama 9 (round 147).

Extends the 33-step run with memory_health, kg_export and
learner_profile (36 steps).
"""
...
def _run() -> dict:
    ...
    batch = fresh_server._call_tool(
        "search_batch",
        {"queries": ["t9-0", "t9-1", "t9-2"], "top_k": 3},
    )
    batch_ok = sum(
        1
...
def main() -> int:
    ...
    parser.add_argument(
        "--out",
        default=os.path.join(_BENCH, "results", "toolchain9_eval.json"),
    )
