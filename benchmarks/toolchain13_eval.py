"""Toolchain panorama 13 (round 167).

Extends the 45-step run with topic_drift_report, forgetting_export and
coverage_report (48 steps).
"""
...
def _run() -> dict:
    ...
    batch = fresh_server._call_tool(
        "search_batch",
        {"queries": ["t13-0", "t13-1", "t13-2"], "top_k": 3},
    )
    batch_ok = sum(
        1
    ...
    pack = fresh_server._call_tool(
        "context_pack",
        {"queries": ["t13-0", "t13-1", "t13-2"], "max_chars": 400},
    )
    pack_ok = int(
...
def main() -> int:
    ...
    parser.add_argument(
        "--out",
        default=os.path.join(_BENCH, "results", "toolchain13_eval.json"),
    )
