"""Final matrix v41 chart: 163 dimensions x 4 projects x 3 models."""
...
def chart() -> str:
    matrix = json.load(
        open(os.path.join(_BENCH, "results", "final_matrix_v41.json"),
             encoding="utf-8")
    )
    ...
    f_txt = _font(16)
    y = 28
    draw.text((42, y), "第 41 期全方位测评：Mnemosis vs GitHub 同类项目（163 维度）",
              fill="#111", font=f_title)
    y += 48
    draw.text((42, y),
              "覆盖第 238-241 轮新增能力（遗忘曲线/情绪衰减/目标进度/工具链28）；"
    ...
    draw.text((42, y + 8),
              ...
              fill="#555", font=f_sub)
    draw.text((42, y + 42),
              "说明：基础维度沿用第 4-40 期真实安装测评；59-241 轮改动已跑统一回归"
              "（en88/zh200/zh10k 及 10k 系列）与一键全测评（149/149）零差异。",
              fill="#555", font=f_sub)

    path = os.path.join(_OUT, "final_full_matrix_v41.png")
    img.save(path)
    return path
