"""Round-167 chart: toolchain panorama 13."""
...
def chart() -> str:
    toolchain = json.load(
        open(os.path.join(_BENCH, "results", "toolchain13_eval.json"),
             encoding="utf-8")
    )
    suite = json.load(
    ...
    draw.text((42, 26), "第 167 轮：工具链全景回归 13（48 步全过）",
              fill="#111", font=f_title)
    draw.text((42, 74),
              "依据：新工具（主题漂移/遗忘曲线/复习覆盖）也要和旧工具在同一份数据上"
              "连续工作，互相不打架。",
              fill="#555", font=f_sub)
    ...
    base_y = 390
    chart_h = 230
    draw.text((x0, 120), "① 48 步工具链结果", fill="#111", font=f_panel)
    draw.line([(x0, base_y), (x0 + 3200, base_y)], fill="#999", width=2)
    for frac, label in ((0.0, "0"), (3.0, "3"), (6.0, "6")):
        y = base_y - frac / 6.0 * chart_h
        draw.line([(x0, y), (x0 + 3200, y)], fill="#e5e5e5", width=1)
    ...
    draw.text((42, 600),
              "怎么看：同一份记忆走完 48 步（含第 164-166 轮的主题漂移/遗忘曲线/"
              "复习覆盖），全部正确；搜索 6/6、批量检索 3/3。",
              fill="#555", font=f_note)
    draw.text((42, 640),
              f"一键全测评扩到 {verified}/{verified}——新增工具没有破坏任何既有机制。",
              fill="#555", font=f_note)
    draw.text((42, 710),
              "回归：253 单元测试全过 + 一键全测评全绿。",
              fill="#555", font=f_note)

    path = os.path.join(_OUT, "round167_toolchain13.png")
    img.save(path)
    return path
