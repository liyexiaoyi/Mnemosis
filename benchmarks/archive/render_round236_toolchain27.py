"""Round-236 chart: toolchain panorama 27 (horizontal, readable)."""
...
def chart() -> str:
    ...
    draw.text((42, 26), "第 236 轮：工具链全景回归 27（90 步全过）",
              fill="#111", font=f_title)
    draw.text((42, 74),
              "同一份记忆连续走完 90 步（含线索/周报/迁移出题），全部正确；"
              "搜索 6/6、批量检索 3/3。",
              fill="#555", font=f_sub)
    max_total = 6
    bar_x0 = 330
    ...
    for i, (name, val, total) in enumerate(steps):
        ...
        y += row_h
    draw.text((42, y + 14),
              "怎么看：90 行全绿——旧功能没坏，第 233-235 轮的线索/周报/"
              "迁移出题也在同一套数据上正常。",
              fill="#555", font=f_note)
    draw.text((42, y + 48),
              "回归：295 单元测试全过 + 一键全测评全绿。",
              fill="#555", font=f_note)

    path = os.path.join(_OUT, "round236_toolchain27.png")
    img.save(path)
    return path
