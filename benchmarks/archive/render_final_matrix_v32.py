"""Final matrix v32 chart: 128 dimensions x 4 projects x 3 models."""
...
def chart() -> str:
    matrix = json.load(
        open(os.path.join(_BENCH, "results", "final_matrix_v32.json"),
             encoding="utf-8")
    )
    ...
    f_txt = _font(16)
    y = 28
    draw.text((42, y), "第 32 期全方位测评：Mnemosis vs GitHub 同类项目（128 维度）",
              fill="#111", font=f_title)
    y += 48
    draw.text((42, y),
              "覆盖第 194-197 轮新增能力（检索路径/记忆圈子/睡前建议/工具链19）；"
    ...
    draw.text((42, y + 8),
              "读法：A 检索矩阵含 122 个新能力维度（Mnemosis 独有，第三方项目不支持）；"
              "B/C 显示三模型作答——换强模型能缩小差距，但检索短板的项目上限仍低。",
              fill="#555", font=f_sub)
    draw.text((42, y + 42),
              "说明：基础 6 维沿用第 4-31 期真实安装测评；59-197 轮改动已跑统一回归"
              "（en88/zh200/zh10k 及 10k 系列）与一键全测评（118/118）零差异。",
              fill="#555", font=f_sub)

    path = os.path.join(_OUT, "final_full_matrix_v32.png")
    img.save(path)
    return path
