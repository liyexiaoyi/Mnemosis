"""Final matrix v31 chart: 124 dimensions x 4 projects x 3 models."""
...
def chart() -> str:
    matrix = json.load(
        open(os.path.join(_BENCH, "results", "final_matrix_v31.json"),
             encoding="utf-8")
    )
    ...
    f_txt = _font(16)
    y = 28
    draw.text((42, y), "第 31 期全方位测评：Mnemosis vs GitHub 同类项目（124 维度）",
              fill="#111", font=f_title)
    y += 48
    draw.text((42, y),
              "覆盖第 189-192 轮新增能力（决策复盘/知识迁移/检索体检/工具链18）；"
    ...
    draw.text((42, y + 8),
              "读法：A 检索矩阵含 118 个新能力维度（Mnemosis 独有，第三方项目不支持）；"
              "B/C 显示三模型作答——换强模型能缩小差距，但检索短板的项目上限仍低。",
              fill="#555", font=f_sub)
    draw.text((42, y + 42),
              "说明：基础 6 维沿用第 4-30 期真实安装测评；59-192 轮改动已跑统一回归"
              "（en88/zh200/zh10k 及 10k 系列）与一键全测评（114/114）零差异。",
              fill="#555", font=f_sub)

    path = os.path.join(_OUT, "final_full_matrix_v31.png")
    img.save(path)
    return path
