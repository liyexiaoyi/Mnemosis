"""Final matrix v17 chart: 67 dimensions x 4 projects x 3 models."""
...
def chart() -> str:
    matrix = json.load(
        open(os.path.join(_BENCH, "results", "final_matrix_v17.json"),
             encoding="utf-8")
    )
    ...
    f_txt = _font(15)
    y = 28
    draw.text((42, y), "第 17 期全方位测评：Mnemosis vs GitHub 同类项目（67 维度）",
              fill="#111", font=f_title)
    y += 48
    draw.text((42, y),
              "覆盖第 119-122 轮新增能力（标签管理/检索日志/清理预览/工具链4）；"
    ...
    draw.text((42, y + 8),
              "读法：A 检索矩阵含 61 个新能力维度（Mnemosis 独有，第三方项目不支持）；"
              "B/C 显示三模型作答——换强模型能缩小差距，但检索短板的项目上限仍低。",
              fill="#555", font=f_sub)
    draw.text((42, y + 42),
              "说明：基础 6 维沿用第 4-16 期真实安装测评；59-122 轮改动已跑统一回归"
              "（en88/zh200/zh10k 及 10k 系列）与一键全测评（57/57）零差异。",
              fill="#555", font=f_sub)

    path = os.path.join(_OUT, "final_full_matrix_v17.png")
    img.save(path)
    return path
