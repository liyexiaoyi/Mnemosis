"""Round-221 chart: toolchain panorama 24 (horizontal, readable)."""
...
def chart() -> str:
    ...
    draw.text((42, 26), "第 221 轮：工具链全景回归 24（81 步全过）",
              fill="#111", font=f_title)
    draw.text((42, 74),
              "同一份记忆连续走完 81 步（含巩固预测/遗忘平衡/元认知），全部正确；"
              "搜索 6/6、批量检索 3/3。",
              fill="#555", font=f_sub)
    max_total = 6
    bar_x0 = 330
    ...
    for i, (name, val, total) in enumerate(steps):
        ...
        y += row_h
    draw.text((42, y + 14),
              "怎么看：81 行全绿——旧功能没坏，第 218-220 轮的巩固预测/遗忘平衡/"
              "元认知也在同一套数据上正常。",
              fill="#555", font=f_note)
    draw.text((42, y + 48),
              "回归：286 单元测试全过 + 一键全测评全绿。",
              fill="#555", font=f_note)

    path = os.path.join(_OUT, "round221_toolchain24.png")
    img.save(path)
    return path
