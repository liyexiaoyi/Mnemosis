"""Final matrix v37 chart: 147 dimensions x 4 projects x 3 models."""
...
def chart() -> str:
    matrix = json.load(
        open(os.path.join(_BENCH, "results", "final_matrix_v37.json"),
             encoding="utf-8")
    )
    ...
    f_txt = _font(16)
    y = 28
    draw.text((42, y), "第 37 期全方位测评：Mnemosis vs GitHub 同类项目（147 维度）",
              fill="#111", font=f_title)
    y += 48
    draw.text((42, y),
              "覆盖第 218-221 轮新增能力（巩固预测/遗忘平衡/元认知/工具链24）；"
    ...
    draw.text((42, y + 8),
              ...
              fill="#555", font=f_sub)
    draw.text((42, y + 42),
              "说明：基础维度沿用第 4-36 期真实安装测评；59-221 轮改动已跑统一回归"
              "（en88/zh200/zh10k 及 10k 系列）与一键全测评（133/133）零差异。",
              fill="#555", font=f_sub)

    path = os.path.join(_OUT, "final_full_matrix_v37.png")
    img.save(path)
    return path
