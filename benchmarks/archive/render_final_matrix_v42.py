"""Final matrix v42 chart: 170 dimensions x 4 projects x 3 models."""
...
def chart() -> str:
    matrix = json.load(
        open(os.path.join(_BENCH, "results", "final_matrix_v42.json"),
             encoding="utf-8")
    )
    ...
    dims = [
        ...
        "curve_fit", "affect_decay", "goal_progress", "toolchain28",
        "retrieval_snapshot", "plan_rehearsal", "math_ladder",
        "physics_simulate", "analogy_prompt", "review_consistency",
        "toolchain29",
    ]
    dim_labels = {
        "en12": "英文12问",
        ...
        "curve_fit": "遗忘曲线(新)",
        "affect_decay": "情绪衰减(新)",
        "goal_progress": "目标进度(新)",
        "toolchain28": "工具链28(新)",
        "retrieval_snapshot": "记忆快照(新)",
        "plan_rehearsal": "计划预演(新)",
        "math_ladder": "数学阶梯(新)",
        "physics_simulate": "物理模拟(新)",
        "analogy_prompt": "类比出题(新)",
        "review_consistency": "复习坚持(新)",
        "toolchain29": "工具链29(新)",
    }
    retr_keys = {
        "en12": "en12",
        ...
        "curve_fit": "curve_fit",
        "affect_decay": "affect_decay",
        "goal_progress": "goal_progress",
        "toolchain28": "toolchain28",
        "retrieval_snapshot": "retrieval_snapshot",
        "plan_rehearsal": "plan_rehearsal",
        "math_ladder": "math_ladder",
        "physics_simulate": "physics_simulate",
        "analogy_prompt": "analogy_prompt",
        "review_consistency": "review_consistency",
        "toolchain29": "toolchain29",
    }
    project_labels = {
        "mnemosis": "Mnemosis",
        "mem0": "mem0官方",
    ...
    f_txt = _font(16)
    y = 28
    draw.text((42, y), "第 42 期全方位测评：Mnemosis vs GitHub 同类项目（170 维度）",
              fill="#111", font=f_title)
    y += 48
    draw.text((42, y),
              "覆盖第 243-249 轮新增能力（记忆快照/计划预演/数学阶梯/物理模拟/"
              "类比出题/复习坚持/工具链29）；作答 = 云端千问 / 本地 qwen2.5:3b / "
              "DeepSeek V4 Flash（我）。",
              fill="#555", font=f_sub)
    y += 44
    ...
    draw.text((42, y + 8),
              ...
              fill="#555", font=f_sub)
    draw.text((42, y + 42),
              "说明：基础维度沿用第 4-41 期真实安装测评；59-249 轮改动已跑统一回归"
              "（en88/zh200/zh10k 及 10k 系列）与一键全测评（160/160）零差异。",
              fill="#555", font=f_sub)

    path = os.path.join(_OUT, "final_full_matrix_v42.png")
    img.save(path)
    return path
