"""Round-249 chart: toolchain panorama 29 (horizontal, readable)."""
...
def chart() -> str:
    toolchain = json.load(
        open(
            os.path.join(_BENCH, "results", "toolchain29_eval.json"),
            encoding="utf-8",
        )
    )
    steps = [
        ...
        ("类比", toolchain["analogy_bridge"], 1),
        ("间隔校准", toolchain["next_interval"], 1),
        ("夜间流程", toolchain["nightly_routine"], 1),
        ("线索", toolchain["cue_diversity"], 1),
        ("周报", toolchain["weekly_review"], 1),
        ("迁移出题", toolchain["transfer_prompt"], 1),
        ("遗忘曲线", toolchain["curve_fit"], 1),
        ("情绪衰减", toolchain["affect_decay"], 1),
        ("目标进度", toolchain["goal_progress"], 1),
        ("记忆快照", toolchain["retrieval_snapshot"], 1),
        ("计划预演", toolchain["plan_rehearsal"], 1),
        ("数学阶梯", toolchain["math_ladder"], 1),
        ("物理模拟", toolchain["physics_simulate"], 1),
        ("类比出题", toolchain["analogy_prompt"], 1),
        ("复习坚持", toolchain["review_consistency"], 1),
    ...
    draw.text((42, 26), "第 249 轮：工具链全景回归 29（99 步全过）",
              fill="#111", font=f_title)
    draw.text((42, 74),
              "同一份记忆连续走完 99 步（含 243-248 轮的 6 个新工具），全部正确；"
              "搜索 6/6、批量检索 3/3。",
              fill="#555", font=f_sub)
    max_total = 6
    bar_x0 = 330
    ...
    for i, (name, val, total) in enumerate(steps):
        ...
        y += row_h
    draw.text((42, y + 14),
              "怎么看：99 行全绿——旧功能没坏，第 243-248 轮的记忆快照/计划预演/"
              "数学阶梯/物理模拟/类比出题/复习坚持也在同一套数据上正常。",
              fill="#555", font=f_note)
    draw.text((42, y + 48),
              "回归：304 单元测试全过 + 一键全测评全绿。",
              fill="#555", font=f_note)

    path = os.path.join(_OUT, "round249_toolchain29.png")
    img.save(path)
    return path
