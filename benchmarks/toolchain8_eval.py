"""Toolchain panorama 8 (round 142).

Extends the 30-step run with interference_report, life_story and
intent_conflicts (33 steps).
"""
...
def _run() -> dict:
    ...
    batch = fresh_server._call_tool(
        "search_batch",
        {"queries": ["t8-0", "t8-1", "t8-2"], "top_k": 3},
    )
    batch_ok = sum(
        1
...
def main() -> int:
    ...
    parser.add_argument(
        "--out",
        default=os.path.join(_BENCH, "results", "toolchain8_eval.json"),
    )
