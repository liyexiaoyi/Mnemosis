"""Round-127 chart: toolchain panorama 5."""
...
def chart() -> str:
    toolchain = json.load(
        open(os.path.join(_BENCH, "results", "toolchain5_eval.json"),
             encoding="utf-8")
    )
    suite = json.load(
    ...
    draw.text((42, 26), "第 127 轮：工具链全景回归 5（18 步全过）",
              fill="#111", font=f_title)
    draw.text((42, 74),
              "依据：新工具（相似体检/网络体检/批量检索）也要和旧工具在同一份数据上"
              "连续工作，互相不打架。",
              fill="#555", font=f_sub)
    ...
    base_y = 390
    chart_h = 230
    draw.text((x0, 120), "① 18 步工具链结果", fill="#111", font=f_panel)
    draw.line([(x0, base_y), (x0 + 1600, base_y)], fill="#999", width=2)
    for frac, label in ((0.0, "0"), (3.0, "3"), (6.0, "6")):
        y = base_y - frac / 6.0 * chart_h
        draw.line([(x0, y), (x0 + 1600, y)], fill="#e5e5e5", width=1)
    ...
    for i, (name, val, total, color) in enumerate(steps):
        bx = x0 + 12 + i * 88
        bh = val / 6.0 * chart_h
        draw.rectangle([bx, base_y - bh, bx + 72, base_y], fill=color)
    ...
    draw.text((42, 600),
              "怎么看：同一份记忆走完 18 步（含第 124-126 轮的相似体检、网络体检、"
              "批量检索），全部正确；搜索 6/6、批量检索 3/3、冲突清零。",
              fill="#555", font=f_note)
    draw.text((42, 640),
              f"一键全测评扩到 {verified}/{verified}——新增索引与体检工具没有破坏任何"
              "既有机制。",
              fill="#555", font=f_note)
    draw.text((42, 710),
              "回归：229 单元测试全过 + 一键全测评全绿。",
              fill="#555", font=f_note)

    path = os.path.join(_OUT, "round127_toolchain5.png")
    img.save(path)
    return path
