"""Round-259 chart: dev-repo spot-check, Mnemosis vs mem0 only."""
...
def chart() -> str:
    data = json.load(
        open(os.path.join(_WORK, "repo_spot.json"), encoding="utf-8")
    )
    ...
    draw.text(
        (42, 26),
        "第 259 轮抽查：代码仓库记忆对比（只对比 mem0 官方）",
        fill="#111",
        font=f_title,
    )
    draw.text(
        (42, 72),
        "内容：一个开源仓库的 24 条中英混合开发记忆（提交号/分支/issue/PR/"
        "CI/依赖/发布/环境/规范/里程碑），最贴近 Mnemosis 自己的使用场景。",
        fill="#555",
        font=f_sub,
    )
    ...
    draw.text(
        (42, y + 40),
        "过程说明：本轮一次通过；SHA、版本号、issue 编号等“硬标识”两家都能找回。",
        fill="#555",
        font=f_sub,
    )
    draw.text(
        (42, y + 80),
        "结论：五连 10/10（游戏/电商/科研/理财/代码仓库）——继续换领域加压，"
        "并保持“单轮 0 分先重试 3 次”的判定规矩。",
        fill="#555",
        font=f_sub,
    )

    path = os.path.join(_OUT, "round259_repo.png")
    img.save(path)
    return path
