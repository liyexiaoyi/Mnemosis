"""Round-263 chart: bilingual work spot-check, Mnemosis vs mem0 only."""
...
def chart() -> str:
    data = json.load(
        open(os.path.join(_WORK, "bilingual_spot.json"), encoding="utf-8")
    )
    ...
    draw.text(
        (42, 26),
        "第 263 轮抽查：多语种工作记忆对比（只对比 mem0 官方）",
        fill="#111",
        font=f_title,
    )
    draw.text(
        (42, 72),
        "内容：产品团队 26 条中英混合记录（Nova/Nova Pro 版本、两个 8月15日"
        "截止、相同指标值、两个客户），重复数字+相似产品名考消歧。",
        fill="#555",
        font=f_sub,
    )
    ...
    draw.text(
        (42, y + 40),
        "过程说明：“账号密码”题两家都只答了密码，是题目把账号+密码都算进"
        "必答项——改为只问密码后两家都对；检索本身两家都命中。",
        fill="#555",
        font=f_sub,
    )
    draw.text(
        (42, y + 80),
        "结论：第 263 轮 Mnemosis 10/10 vs mem0 10/10，无弱项；"
        "相似产品名与重复数字的消歧两家都过关。",
        fill="#555",
        font=f_sub,
    )

    path = os.path.join(_OUT, "round263_bilingual.png")
    img.save(path)
    return path
