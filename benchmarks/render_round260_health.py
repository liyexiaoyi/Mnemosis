"""Round-260 chart: health-records spot-check, Mnemosis vs mem0 only."""
...
def chart() -> str:
    data = json.load(
        open(os.path.join(_WORK, "health_spot.json"), encoding="utf-8")
    )
    ...
    draw.text(
        (42, 26),
        "第 260 轮抽查：健康医疗记忆对比（只对比 mem0 官方）",
        fill="#111",
        font=f_title,
    )
    draw.text(
        (42, 72),
        "内容：26 条健康医疗记忆，埋了时间干扰题（多次血压/血糖，问“上次/最近”"
        "必须找回最新那条）；另有用药/过敏/疫苗等干扰项。",
        fill="#555",
        font=f_sub,
    )
    ...
    draw.text(
        (42, y + 40),
        "过程说明：mem0 在“用药记录”上真实漏检——前 4 条全是血压记录和阿司匹林，"
        "没有氨氯地平，作答只能答“不知道”；Mnemosis 全部找回。",
        fill="#555",
        font=f_sub,
    )
    draw.text(
        (42, y + 80),
        "结论：本轮 Mnemosis 10/10 领先 mem0 9/10（首次出现我方严格占优）；"
        "按规则无需优化，继续换领域验证。",
        fill="#555",
        font=f_sub,
    )

    path = os.path.join(_OUT, "round260_health.png")
    img.save(path)
    return path
