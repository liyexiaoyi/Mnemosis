"""Round-258 chart: personal-finance spot-check, Mnemosis vs mem0 only."""
...
def chart() -> str:
    data = json.load(
        open(os.path.join(_WORK, "finance_spot.json"), encoding="utf-8")
    )
    ...
    draw.text(
        (42, 26),
        "第 258 轮抽查：个人理财记忆对比（只对比 mem0 官方）",
        fill="#111",
        font=f_title,
    )
    draw.text(
        (42, 72),
        "内容：家庭账本的 24 条记忆（账户/工资/水电/预算/持仓/贷款/保险/"
        "订阅/目标），干扰项更多、提问更模糊；Mnemosis 与 mem0ai 2.0.17 同库同题。",
        fill="#555",
        font=f_sub,
    )
    ...
    draw.text(
        (42, y + 40),
        "过程说明：首轮 Mnemosis 的“月度总结”被判 0，但同一上下文重试 3 次全对——"
        "是单轮模型波动而非检索漏；另一题是题目口径过严，修正后两家都对。",
        fill="#555",
        font=f_sub,
    )
    draw.text(
        (42, y + 80),
        "结论：四连 10/10（游戏制作/电商/科研/理财）——同时给抽查加了一条规矩："
        "单轮 0 分必须重试 3 次确认，不能把模型噪音当成系统弱项。",
        fill="#555",
        font=f_sub,
    )

    path = os.path.join(_OUT, "round258_finance.png")
    img.save(path)
    return path
