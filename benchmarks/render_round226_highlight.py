"""Round-226 highlight chart (simple, for non-experts)."""
...
def chart() -> str:
    ...
    draw.text((42, 26), "第 226 轮大白话版：84 步全过", fill="#111", font=f_title)
    draw.text(
        (42, 74),
        "同一份记忆连续走完 84 个动作，全部正确；新加 3 个工具也不打架。",
        fill="#555",
        font=f_sub,
    )
    ...
    draw.text(
        (42, 700),
        "回归：289 个单元测试全过 + 一键全测评全绿。",
        fill="#555",
        font=f_note,
    )

    path = os.path.join(_OUT, "round226_toolchain25_simple.png")
    img.save(path)
    return path
