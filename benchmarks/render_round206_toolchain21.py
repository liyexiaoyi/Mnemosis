"""Round-206 chart: toolchain panorama 21 (horizontal, readable)."""
...
def chart() -> str:
    ...
    draw.text((42, 26), "第 206 轮：工具链全景回归 21（72 步全过）",
              fill="#111", font=f_title)
    draw.text((42, 74),
              "同一份记忆连续走完 72 步（含推理链/目标重放），全部正确；"
              "搜索 6/6、批量检索 3/3。",
              fill="#555", font=f_sub)
    max_total = 6
    bar_x0 = 330
    ...
    for i, (name, val, total) in enumerate(steps):
        ...
        y += row_h
    draw.text((42, y + 14),
              "怎么看：72 行全绿——旧功能没坏，第 204-205 轮的推理链/目标重放"
              "也在同一套数据上正常。",
              fill="#555", font=f_note)
    draw.text((42, y + 48),
              "回归：277 单元测试全过 + 一键全测评全绿。",
              fill="#555", font=f_note)

    path = os.path.join(_OUT, "round206_toolchain21.png")
    img.save(path)
    return path
