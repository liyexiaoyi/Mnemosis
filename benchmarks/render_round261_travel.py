"""Round-261 chart: travel-planning spot-check, Mnemosis vs mem0 only."""
...
def chart() -> str:
    data = json.load(
        open(os.path.join(_WORK, "travel_spot.json"), encoding="utf-8")
    )
    ...
    draw.text(
        (42, 26),
        "第 261 轮抽查：出行旅行记忆对比（只对比 mem0 官方）",
        fill="#111",
        font=f_title,
    )
    draw.text(
        (42, 72),
        "内容：东京旅行的 26 条记忆（行程/机票/酒店/签证/行李/交通/预约/"
        "语言/保险/预算/天气），含“航班改签”这类新旧信息并存题。",
        fill="#555",
        font=f_sub,
    )
    ...
    draw.text(
        (42, y + 40),
        "过程说明：首轮 Mnemosis 在“机票酒店”答 0——原始机票记录排在第 5 位，"
        "只带回改签通知；新增“实体锚点检索”把带航班号的原始记录补进前 4，"
        "复测后 Mnemosis 10/10、mem0 10/10。",
        fill="#555",
        font=f_sub,
    )
    draw.text(
        (42, y + 80),
        "结论：发现我方真实弱项（编号类问题漏原始记录）→ 实现通用机制 → "
        "复测补齐至平局。309 个单测、163/163 全测评零回归。",
        fill="#555",
        font=f_sub,
    )

    path = os.path.join(_OUT, "round261_travel.png")
    img.save(path)
    return path
