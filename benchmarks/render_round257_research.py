"""Round-257 chart: research-notes spot-check, Mnemosis vs mem0 only."""
...
def chart() -> str:
    data = json.load(
        open(os.path.join(_WORK, "research_spot.json"), encoding="utf-8")
    )
    ...
    draw.text(
        (42, 26),
        "第 257 轮抽查：科研笔记记忆对比（只对比 mem0 官方）",
        fill="#111",
        font=f_title,
    )
    draw.text(
        (42, 72),
        "内容：一位研究者的 18 条中英混合科研笔记（论文/引用/实验/数据集/"
        "截稿/代码/合作者/审稿/基金）；Mnemosis 与 mem0ai 2.0.17 同库同题。",
        fill="#555",
        font=f_sub,
    )
    ...
    draw.text(
        (42, y + 40),
        "过程说明：本轮一次通过，无超时、无题目口径返工；"
        "中英混合内容（DOI/arxiv/代码路径）两家都能准确找回。",
        fill="#555",
        font=f_sub,
    )
    draw.text(
        (42, y + 80),
        "结论：三连 10/10（游戏制作、电商运营、科研笔记）——继续换领域，"
        "用更难的内容找差距。",
        fill="#555",
        font=f_sub,
    )

    path = os.path.join(_OUT, "round257_research.png")
    img.save(path)
    return path
