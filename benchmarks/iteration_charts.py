...
def retrieval_chart() -> None:
    ...
    rows = [
        ...
        ("半线索回忆@5", locomo_pct(locomo, "keyword", "partial", "hit5"),
         locomo_pct(locomo, "ngram", "partial", "hit5")),
        ("干扰项拒答", 100.0, 100.0),
    ]
    w, h = 820, 400
    ...
    lines = _svg_open(
        "检索能力实测（24 会话 / 188 题，命中率 %）",
        "“时序·下一事件”= 问“之后发生了什么”能否找到下一条；“半线索回忆”= 只给一半线索能否找到；“干扰项拒答”= 没聊过的话题会不会乱编",
        w, h,
    )
