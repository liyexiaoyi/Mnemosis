"""Toolchain panorama 14 (round 172).

Extends the 48-step run with source_calibration, forgetting_risk and
bridge_suggestions (51 steps).
"""
...
def _run() -> dict:
    ...
    batch = fresh_server._call_tool(
        "search_batch",
        {"queries": ["t14-0", "t14-1", "t14-2"], "top_k": 3},
    )
    batch_ok = sum(
        1
    ...
    pack = fresh_server._call_tool(
        "context_pack",
        {"queries": ["t14-0", "t14-1", "t14-2"], "max_chars": 400},
    )
    pack_ok = int(
...
def main() -> int:
    ...
    parser.add_argument(
        "--out",
        default=os.path.join(_BENCH, "results", "toolchain14_eval.json"),
    )
