"""Stdlib-only SVG rendering for memory visualizations."""
...
def render_memory_map_svg(data: dict, title: str) -> str:
    """Render ``memory_map()`` output as a human-readable SVG chart.

    Bar chart of the top topics (count, retrievability, importance) plus
    a weak/ok/strong strength histogram. Pure stdlib, Chinese labels.
    """
    topics = data["topics"][:20]
    strength = data["strength"]
    width, pad, row_h = 900, 24, 30
    height = pad * 2 + 40 + len(topics) * row_h + 110
    max_count = max([t["count"] for t in topics] or [1]) or 1
    parts = [
        (
    ...
    max_strength = max([strength[k] for k, _ in labels] or [1]) or 1
    parts.append(
